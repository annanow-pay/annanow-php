...
$gateway = new \Annanow\Models\Request\Gateway();
...
// optional: payment service provider(s) to use (see http://developers.annanow-pay.com/docs/miscellaneous)
...
$gateway->addField($type = 'email', $value = 'max.muster@annanow-pay.com');
...
try {
    $response = $annanow->create($gateway);
    var_dump($response);
