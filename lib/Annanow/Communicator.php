...
class Communicator
{
    ...
    const API_URL_BASE_DOMAIN = 'annanow-pay.com';
    const DEFAULT_COMMUNICATION_HANDLER = '\Annanow\CommunicationAdapter\CurlCommunication';
    ...
    /**
     * @var string The Annanow instance name.
     */
    protected $instance;
    /**
    ...
    public function __construct($instance, $apiSecret, $communicationHandler, $apiBaseDomain)
    {
        ...
        if (!class_exists($communicationHandler)) {
            throw new AnnanowException('Communication handler class ' . $communicationHandler . ' not found');
        }
     ...
     * @param \Annanow\Models\Base $model  The model which has the same functionality like a filter.
     *
     * @return \Annanow\Models\Base[]|\Annanow\Models\Base An array of models or just one model which
     *                                                                       is the result of the API call
     * @throws \Annanow\AnnanowException An error occurred during the Annanow Request
     */
    public function performApiRequest($method, \Annanow\Models\Base $model)
    {
        ...
        if (!isset($response['body']['data']) || !is_array($response['body']['data'])) {
            if (!isset($response['body']['message'])) {
                throw new \Annanow\AnnanowException('Annanow PHP: Configuration is wrong! Check instance name and API secret', $response['info']['http_code']);
            }
            throw new \Annanow\AnnanowException($response['body']['message'], $response['info']['http_code']);
        }
     ...
     * @throws \Annanow\AnnanowException The method is not implemented yet.
     */
